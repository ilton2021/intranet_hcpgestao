@extends('layouts.adm')
	...
	<div class="row" style="margin-top: 0px;">
		<div class="col-md-12 text-center">
			<h3 style="font-size: 18px;">CADASTRAR NOVA PERMISSÃO:</h3>
		</div>
	</div>
	@if ($errors->any())
		 ...
		 <div class="accordion" id="accordionExample">
                <div class="card">
                    <a class="card-header bg-success text-decoration-none text-white bg-success" type="button" data-toggle="collapse" data-target="#PESSOAL" aria-expanded="true" aria-controls="PESSOAL">
                        Permissões: <i class="fas fa-check-circle"></i>
                    </a>
                </div>	
					<form action="{{\Request::route('storePermissoes')}}" method="post">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<table border="0" class="table-sm" style="line-height: 1.5;" >
						 <tr>
							<td> Tela: </td>
							<td>
								<input class="form-control" style="width: 400px" type="text" id="tela" name="tela" required value="{{ old('tela') }}" />
							</td>
						 </tr>
						 <tr>
							<td><input hidden type="text" id="tela_" name="tela_" class="form-control" value="novo_permissoes" /></td>
